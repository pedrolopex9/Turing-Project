...
def run_executable_int(exe_path, params):
    """
    Executa o programa com parâmetros INTEIROS e extrai o valor numérico.
    """
    try:
        # Garante que todos os parâmetros são inteiros
        int_params = [str(int(p)) for p in params]
        cmd = [exe_path] + int_params
        ...
        print(f"Params: {int_params} -> Output: {output.strip()}")
...
def particle_swarm_optimization(
    ...
    # Gera população inicial INTEIRA
    particles = []
    ...
    for i, p in enumerate(particles):
        val = run_executable_int(exe_path, [x1_text] + p.tolist())
        personal_best_values.append(val)
        print(f"Partícula {i}: {p} -> {val}")
    ...
    print(f"Melhor inicial: {global_best} -> {global_best_value}")
    ...
    for it in range(iterations):
        ...
        for i in range(num_particles):
            ...
            # Avalia nova posição
            val = run_executable_int(exe_path, [x1_text] + particles[i].tolist())
